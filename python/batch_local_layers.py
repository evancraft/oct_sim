...
def kill_pcbs(effs, nKillPCBs = 0, dead_PCB_eff = 0, stepEff = 0):
	killed = []
	for real in range(127):
		effs[real] = RealEff[real] # sets real efficiencies
	for a in range(8):
		pcb = random.randint(0,15) #layer 1
		while pcb in killed:
			pcb = random.randint(0,15)
		killed.append(pcb)
		effs[pcb] = 0 + stepEff
	for b in range(0):
		pcb = random.randint(16,31) #layer 2
		while pcb in killed:
			pcb = random.randint(16,31)
		killed.append(pcb)
		effs[pcb] = 0 + stepEff
	for c in range(8):
		pcb = random.randint(32,47) #layer 3
		while pcb in killed:
			pcb = random.randint(32,47)
		killed.append(pcb)
		effs[pcb] = 0 + stepEff
	for d in range(0):
		pcb = random.randint(48,63) #layer 4
		while pcb in killed:
			pcb = random.randint(48,63)
		killed.append(pcb)
		effs[pcb] = 0 + stepEff
	for e in range(0):
		pcb = random.randint(64,79) #layer 5
		while pcb in killed:
			pcb = random.randint(64,79)
		killed.append(pcb)
		effs[pcb] = 0 + stepEff
	for f in range(0):
		pcb = random.randint(80,95) #layer 6
		while pcb in killed:
			pcb = random.randint(80,95)
		killed.append(pcb)
		effs[pcb] = 0 + stepEff
	for g in range(0):
		pcb = random.randint(96,111) #layer 7
		while pcb in killed:
			pcb = random.randint(96,111)
		killed.append(pcb)
		effs[pcb] = 0 + stepEff
	for h in range(0):
		pcb = random.randint(112,127) #layer 8
		while pcb in killed:
			pcb = random.randint(112,127)
		killed.append(pcb)
		effs[pcb] = 0 + stepEff
	if nKillPCBs == -1:
		effs = []
		for i in range(8):
			pcb = int(round(random.randint(0,15)))
			x = [0]*16
			x[pcb] = 1
			effs += x
...
def make_jobs(nKillPCBJobs = 0, # Number of different configurations of randomly killed PCBs
	...
	for j in range(nKillPCBJobs):
		...
		job['efficiencies'] = formatMMEffString(kill_pcbs(effs,nKillPCBs,dead_PCB_eff, stepEff))
...
# Input: None
# Output: None
def main(nKillPCBJobs = 1, nKillPCBs = 16, fullEff = 1.0, stepEff = .1, outDir = "/Users/evancraft/Documents/"):
	jobs = make_jobs(nKillPCBJobs = nKillPCBJobs, nKillPCBs = nKillPCBs, fullEff = fullEff, stepEff = stepEff, outDir = outDir)
...
def loop():
	...
	for i in range(10):#np.linspace(0,1,11):
		outDir = "/Users/evancraft" + "/stepEff%.2f"%stepEff
		os.mkdir(outDir)
		main(nKillPCBJobs=3,nKillPCBs=20,fullEff=round(fullEff,2), stepEff = round(stepEff,2), outDir=outDir)
		stepEff = stepEff + .1;
